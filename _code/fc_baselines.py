...
🔴 设计依据(2026-09-04 用户拍板 + petro-knowledge/05-生产优化.md 的「平凡基线」节):
   ...
   正确做法是用领域内公认的平凡基线。本文件实现其中三条:
     B2  一维均匀缩放  —— 所有注水井同乘 alpha，扫描取最优。1 个决策变量。
                          ...
                          "高维井控优化"的整个卖点就不成立。
     B4  等预算随机搜索  🔴 2026-09-06 退役,不参与主线对比(见 _legacy/)
         —— 与 Agent 臂**完全相同**的模拟预算内均匀抽样取最优。
                          检验"你的智能体到底有没有用"。
     BG  贪心/短期最优    🔴 2026-09-06 退役,不参与主线对比(见 _legacy/)
          —— 逐时段选当下产油最优，不看长期。
                          对应用户说的"短期最优/贪心算法"。
...
def main() -> int:
    ...
    ap.add_argument("--method", choices=["B2", "B4", "BG", "all"], default="B2")
